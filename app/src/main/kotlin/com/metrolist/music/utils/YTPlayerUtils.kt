...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        // Try Subsonic first if available
        try {
            val subsonicResult = SubsonicPlayerUtils.playerResponseForPlayback(
                songId = videoId,
                audioQuality = audioQuality,
                connectivityManager = connectivityManager
            )
            if (subsonicResult.isSuccess) {
                Timber.tag(logTag).d("Using Subsonic stream URL")
                val data = subsonicResult.getOrThrow()
            ...
            Timber.tag(logTag).d("Subsonic not available or failed, falling back to YouTube: ${e.message}")
            // Fall through to YouTube implementation
        }
        /**
